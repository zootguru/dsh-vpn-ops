...
export function getTarget(config: ResolvedConfig, targetId: string): ResolvedTargetConfig {
  const target = config.targets.find(candidate => candidate.id === targetId)
  if (!target) throw new Error(`unknown targetId "${targetId}"; use vpn_targets to list the allowlist`)
  return target
}

function resolveTarget(target: TargetConfig): ResolvedTargetConfig {
  ...
  if (target.identityFile === target.knownHostsFile) throw new Error(`${target.id}: identityFile and knownHostsFile must differ`)
  ...
  ] as const) assertRemotePath(`${target.id}.${field}`, value)
  ...
  if (serverNetwork.prefix > 30) throw new Error(`${target.id}.wireguardAddress must leave room for clients`)

  const clients = target.clients.map(client => resolveClient(target.id, client, serverNetwork))
  if (clients.length === 0) throw new Error(`${target.id}.clients must contain at least one client`)
...
function resolveClient(targetId: string, client: ClientConfig, server: Ipv4Cidr): ResolvedClientConfig {
  ...
  if (address.prefix !== 32) throw new Error(`${targetId}.${client.id}.wireguardAddress must use /32`)
  ...
  if (address.value === server.value) throw new Error(`${targetId}.${client.id}.wireguardAddress duplicates the server address`)
...
function parseIpv4Cidr(field: string, value: string): Ipv4Cidr {
  const match = /^(\d{1,3}(?:\.\d{1,3}){3})\/(\d{1,2})$/.exec(value)
  if (!match) throw new Error(`${field} must be an IPv4 CIDR`)
  const ip = match[1]
  const prefix = Number(match[2])
  if (ip === undefined || !Number.isInteger(prefix) || prefix < 0 || prefix > 32) throw new Error(`${field} has an invalid prefix`)
  const numeric = ipv4ToNumber(field, ip)
  const mask = prefix === 0 ? 0 : (0xffff_ffff << (32 - prefix)) >>> 0
  return { value: numeric, prefix, mask }
}

function assertIpv4(field: string, value: string): string {
  ipv4ToNumber(field, value)
  return value
}

function ipv4ToNumber(field: string, value: string): number {
  const parts = value.split('.')
  if (parts.length !== 4) throw new Error(`${field} must be an IPv4 address`)
  let result = 0
  for (const part of parts) {
    if (!/^\d{1,3}$/.test(part)) throw new Error(`${field} must be an IPv4 address`)
    const octet = Number(part)
    if (octet < 0 || octet > 255) throw new Error(`${field} must be an IPv4 address`)
    result = ((result << 8) | octet) >>> 0
  }
  return result
}

function formatNetwork(value: Ipv4Cidr): string {
  const network = value.value & value.mask
  return `${[(network >>> 24) & 255, (network >>> 16) & 255, (network >>> 8) & 255, network & 255].join('.')}/${value.prefix}`
}

function assertDestination(field: string, value: string): void {
  const match = /^([A-Za-z0-9.-]+):(\d{1,5})$/.exec(value)
  if (!match || !SAFE_SERVER_NAME.test(match[1] ?? '')) throw new Error(`${field} must be a DNS name followed by :port`)
  positiveInteger(`${field} port`, Number(match[2]), 65_535)
}

function assertAbsoluteLocalPath(field: string, value: string): void {
  if (!isAbsolute(value) || value.includes('\0')) throw new Error(`${field} must be an absolute local path`)
}

function assertRemotePath(field: string, value: string): void {
...
function assertFileSuffix(field: string, value: string, suffix: string): void {
  if (!value.endsWith(suffix)) throw new Error(`${field} must name a ${suffix} file`)
}

function assertMatch(field: string, value: string, pattern: RegExp): void {
  if (!pattern.test(value)) throw new Error(`${field} contains unsupported characters or has an invalid shape`)
}

function positiveInteger(field: string, value: number, maximum: number): number {
  return integerRange(field, value, 1, maximum)
}

function integerRange(field: string, value: number, minimum: number, maximum: number): number {
