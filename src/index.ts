...
export function registerTools(ctx: Context, operations: VpnOperationsApi): void {
...
function targetParameter() {
  return { type: 'string', required: true, description: 'Allowlisted target id from vpn_targets.' } as const
}
...
function targetCard(title: string, kind: 'read' | 'search' | 'execute', targetId: string) {
  return { card: 'generic' as const, title: `${title}: ${targetId}`, kind, rawInput: targetId }
}
